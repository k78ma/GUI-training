...
class Test():
    ...
    def get_accuracy_test(self, test_loader, model, device, epoch):
        ...
        test_loss, outputs, label_list, FP, FN, PGI = self.cal_correct(test_loader, model, device)
        ...
        return test_loss, outputs, label_list, FP, FN, PGI
    ...
    def cal_correct(self, loader, model, device):
        ...
        PGI = np.mean(A_cagr)/np.mean(B_cagr)
        print('PGI:')
        print(PGI)
        ...
        #root.fp_plot.scatter(epoch, fp_rate, color = '#88c999')
        #root.fn_plot.scatter(epoch, fn_rate, color = '#88c999')



        return sum_loss/total, pred_list, label_list, TP, FP, PGI
