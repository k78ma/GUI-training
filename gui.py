...
class GUI:
    ...
    def __init__(self, root):
        ...
        self.uploadLabel.place(x = 20, y = 77.5, anchor = 'nw')
      
        self.uploadInput = tk.StringVar()
        self.uploadInputBox = tk.Entry(self.main_frame, textvariable=self.uploadInput, width=51)
        self.uploadInputBox.place(x=150, y = 77.5)

        self.uploadButton = tk.Button(self.main_frame, text = 'Browse', command=lambda: self.button_action('Upload'), bg='#283B5B', fg='white')
        self.uploadButton.place(x = 530, y = 75, anchor = 'nw', width=55)

        # Choose output directory
        self.outdirLabel = tk.Label(self.main_frame, text='Log directory:', bg='#64778D', fg='white', width=16, anchor = 'w')
        self.outdirLabel.place(x = 20, y = 120, anchor = 'nw')
        ...
        # Model output
        self.outModelLabel = tk.Label(self.main_frame, text='Output model:', bg='#64778D', fg='white', width=16, anchor = 'w')
        self.outModelLabel.place(x = 20, y = 160, anchor = 'nw')

        self.outModelInput = tk.StringVar()
        ...
        # Restart training
        restartTrainButton = tk.Button(self.main_frame, text = 'Restart', command = lambda:self.button_action("Restart"), bg='#283B5B', fg='white')
        ...
        # Start year
        self.pretrained_value = tk.StringVar()
        self.pretrained_Label = tk.Label(self.param_frame, text='Pre-trained:', bg='#64778D', fg='white', width=14, anchor = 'w')
        self.pretrained_Label.place(x = 30, y = 240, anchor = 'nw')
        self.pretrained_Entry = ttk.Spinbox(self.param_frame, values=("True","False"), textvariable=self.pretrained_value, wrap = True, width=47)
        self.pretrained_Entry.place(x = 140, y = 240)
    ...
    def paramButtonFunction(self, args):
        ...
        if args == 'Set params':
            ...
            self.param_dict["pretrained"] = self.pretrained_Entry.get()
        ...
        if args == "Use default":
            ...
            self.pretrained_Entry.delete(0, 'end')
            
            self.epoch_SpinBox.insert(0, 50)
            self.lr_Entry.insert(0, "0.0001")
            self.batch_SpinBox.insert(0, 8)
            self.val_ratio_Entry.insert(0, "0.4")
            self.pretrained_Entry.insert(0, "False")
            ...
            self.param_dict["pretrained"] = False
    ...
    def button_action(self,args):
        if args == 'Save name':
            self.root.title('Training GUI - ' + self.outModelInput.get())
        if args == 'Configure':
            self.param_window()
        if args == 'Upload':
            self.select_file()
